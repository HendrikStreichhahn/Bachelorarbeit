...
\section{Monitorability}
	...
	\end{definition}
		...
		\begin{definition}[Delay Generator]
			...
			\end{itemize}
		\end{definition}
		The definition of the \textit{Delay Generator} is visualized in figure~\ref{fig:DelayGenerator}. On the left side is the initial state $q_0$. The first input leads to a transition to the wait state of the corresponding input state. The clock $c$ is resetted in this transition.\\
	...
	\begin{figure}
		\begin{tikzpicture}
			...
			\node[below of=delayRight] (stateDelayRight){$\mathbb{D}_{state}\cup\{Delay\}\times\mathbb{T}$};
	...
	\begin{figure}
		\begin{tikzpicture}[state/.style={draw, circle,minimum size=1.5cm, node distance=3cm}]
			\node[state, initial] at (0,0)(qstart)  {$q_{0}$};
