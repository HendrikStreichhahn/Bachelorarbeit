...
\label{comparisonConstraints}
	As said before, the \emph{TADL2 Timing Constraints} and the \emph{AUTOSAR Timing Extension} are compatible in parts and many of the \emph{AUTOSAR Timing Extension} can be noted as equivalent combinations of the \emph{TADL2 Timing Constraints}. In \cite{TIMMO2USE}, the relation between these constraints is shown. For the sake of completeness, each of the \emph{AUTOSAR Timing Extension} will be listed in the following and the relations, that are described in \cite{TIMMO2USE}, will explained and amended.
	% TODO -> autosar has been updated after release of TADL2-> comparison in \cite{TIMMO2USE} is outdated in some points
	% TODO-> MultidimensionalTime <->
	% TODO Autosar types (scope, chains, events, executables,...)
	...
	\subsubsection{SporadicEventTriggering}
		AUTOSARs \emph{SporadicEventTriggering} with the attributes $(\textcolor{gray}{event}, jitter, maximumInterArrivalTime,  minimumInterArrivalTime, period)$ is equivalent to the \emph{TADL2} \emph{SporadicConstraint}, but the names of the attributes are different:\\
		\begin{math}
			lower=period\\
	...
	\subsubsection{ExecutionOrderConstraint}
		The goal of AUTOSAR Timing Extension \emph{ExecutionOrderConstraint} is to describe the order of events or the execution order of executables. 
