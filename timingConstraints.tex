...
\subsection{TADL2-Timing Constraints}
	...
	\subsubsection{PatternConstraint}
		...
		There is a major weak spot in the definition of this constraint, because the set $X$ can be set to the empty set. In this case, the part of the definition, which uses the \emph{PeriodicConstraint} and the \emph{DelayContraint}, are always satisfied, irrespective of the event in $event$. Therefore, this constraint only ensures the minimal distance between two events, what should not the purpose of it. To avoid this weakness, $X$ is restricted to non-empty sets for the rest of this thesis.
